...
class XPlaneConnect(object):
    ...
    # Basic Functions
    def __init__(self, xpHost='localhost', xpPort=49009, port=8081, timeout=100):
    ...
    def sendCOMM(self, command):
        # Preconditions
        # Pack message
        header = b'CMND'
        comm = command.encode()
        buffer = struct.pack('<4sx500s', header, comm)
        # Send
        self.socket.sendto(buffer, 0, ("127.0.0.1", 49000))
        #self.comm_sock.sendto(buffer, (self.comm_beacon['ip'], self.comm_beacon['port']))
